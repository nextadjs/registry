import type { AdCOMContext } from "@/types/adcom";
import type { BuyerConfig, BuyerOpenRTB2Spec, BuyerUserConfig, ClientBuyerSpec } from "./types";
import type {
  BuyerOpenRTB2SpecWrapper,
  ClientBuyerSpecWrapper as IClientBuyerSpecWrapper,
} from "./types/spec-wrapper";
import type { BuyerOpenRTBSpecWrapper } from "./types/spec-wrapper";
import type {
  V26Bid,
  V26BidRequest,
  V26BidResponse,
  V26Imp,
} from "@/types/openrtb";
import type { DefaultParams } from "@/types";
import { ClientSignal, ClientSignalSpecWrapper } from "@/signal";

export class ClientBuyerSpecWrapper<TParams extends DefaultParams> implements IClientBuyerSpecWrapper<TParams> {
  public constructor(
    private readonly _config: BuyerConfig,
    private readonly _userConfig: BuyerUserConfig<TParams>,
    private readonly _spec: ClientBuyerSpec<TParams>
  ) {}

  public async signals() {
    return (this._spec.signals || []).map((signal) => {   
       return new ClientSignal();
    }); 
  }

  public compliances() {
    return [];
  }

  public openrtb(
    context: AdCOMContext
  ): BuyerOpenRTBSpecWrapper<V26BidRequest> {
    const specWrap: BuyerOpenRTBSpecWrapper< V26BidRequest> = {};
    const { openrtb } = this._spec;

    if (openrtb?.v26) {
      specWrap.v26 = this.makeOpenRTB26SpecWrapper(openrtb.v26, context);
    }

    return specWrap;
  }

  private makeOpenRTB26SpecWrapper<
    TReq extends V26BidRequest,
    TImp extends V26Imp,
    TRes extends V26BidResponse,
    TBid extends V26Bid
  >(
    spec: BuyerOpenRTB2Spec<TReq, TImp, TRes, TBid, any, AdCOMContext>,
    context: AdCOMContext
  ): BuyerOpenRTB2SpecWrapper<TReq, TImp, TRes, TBid> {
    const contextSpec = this.getContextSpecificOpenRTB(context)?.v26;

    return {
      configureRequestDetails: () => {
        if (contextSpec?.configureRequestDetails) {
          return contextSpec.configureRequestDetails(
            this._userConfig.params,
            context as any
          );
        }

        return spec.configureRequestDetails(this._userConfig.params, context);
      },

      decorateBidRequest: async (request: TReq): Promise<TReq> => {
        if (contextSpec?.decorateBidRequest) {
          return await contextSpec.decorateBidRequest(
            request as any,
            this._userConfig.params,
            context as any
          ) as TReq;
        }
      
        if (spec?.decorateBidRequest) {
          return await spec.decorateBidRequest(
            request,
            this._userConfig.params,
            context
          );
        }
        return request;
      },

      decorateImpression: async (impression: TImp) => {
        if (contextSpec?.decorateImpression) {
          return await contextSpec.decorateImpression(
            impression as any,
            this._userConfig.params,
            context as any
          ) as TImp;
        }

        if (spec?.decorateImpression) {
          return await spec.decorateImpression(
            impression,
            this._userConfig.params,
            context
          );
        }
        return impression;
      },

      decorateBidResponse: async (response: TRes) => {
        if (contextSpec?.decorateBidResponse) {
          return await contextSpec.decorateBidResponse(
            response as any,
            this._userConfig.params,
            context as any
          ) as TRes;
        }

        if (spec?.decorateBidResponse) {
          return await spec.decorateBidResponse(
            response,
            this._userConfig.params,
            context
          );
        }
        return response;
      },

      decorateBid: async (bid: TBid) => {
        if (contextSpec?.decorateBid) {
          return await contextSpec.decorateBid(
            bid as any,
            this._userConfig.params,
            context as any
          ) as TBid;
        }

        if (spec?.decorateBid) {
          return await spec.decorateBid(bid, this._userConfig.params, context);
        }
        return bid;
      },
    };
  }

  private getContextSpecificOpenRTB(context: AdCOMContext) {
    if ("site" in context && this._spec.site?.openrtb) {
      return this._spec.site.openrtb;
    }
    if ("app" in context && this._spec.app?.openrtb) {
      return this._spec.app.openrtb;
    }
    if ("dooh" in context && this._spec.dooh?.openrtb) {
      return this._spec.dooh.openrtb;
    }
  }
}
